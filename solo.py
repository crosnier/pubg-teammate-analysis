...
async def _run(playername):
    ...
    if brief:
        ...
        pathing = compute_match_pathing(player_id, latest_match_id, latest_events)
        pathing_line = format_match_pathing_line(pathing)

    print("\n\n")
    render_solo_profile(playername, archetype, headline, combat_stats, coaching, brief,
                         killer_intel_line, drop_zone_line, flow_line, pathing_line)
